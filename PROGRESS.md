...
- 2026-08-05 — **Admin round 2 + chat glass pass** (`454b246`→`116f15a`). Admin: KPI sparklines removed and the figures scaled up (1.55→2.15rem), which exposed two silent stage-2 bugs — four `#*-pending` ids still being written to after that tile became Group chats, and a `#spark-groups` box nothing rendered into; range toggle moved inside the chart; whole page scaled by raising the root 16→18px (43 of its font sizes sat under 12px) with phones held at 16; **Overview** became its own rail tab and the default landing panel; Login Monitor cards expand to a 30-day per-user history from a new `GET /api/admin/login-activity/:userId/series` (zero-filled days, id validated, range clamped 7–90). Chat: one shared glass recipe in `:root` replacing two hand-picked blurs, 24px panel radius, the `LOCAL. ACADEMIC. PERSISTENCE.` strip, an **Unread** filter pill, a faded logo disc, and a **Settings** panel with change-password (`PUT /api/user/password`, verifies the current password, bumps `token_version`) and an editable display name. New tests 5.9–5.18, 6.13, 2.11–2.13, 9.8–9.11
- 2026-08-06 — **Admin round 3 + feedback comments.** Admin: KPI tiles are now buttons that select which metric the hero chart plots (`/stats/trends` gained a `totalGroups` series so all five can); the audit log gained action/date/user filters, search and server-side paging over 27,000 rows, with a "security events only" switch because ~70% of the table is the admin's own `VIEW_*` reads; a broadcast composer reaching the previously-unreachable `broadcast:send` capability via `POST /api/admin/broadcast`; group listing, member detail and delete; and a Download-backup button (`VACUUM INTO`, filename-pattern + resolved-path guard on the download). Feedback form: an optional **Section E comment box** (1000 chars), stored in a new `survey_responses.notes` column, shown in the admin response detail and included in the CSV export — the Likert means say how highly people rated MSUkaIP, the comment is the part Chapter 4 can quote. New tests 5.19–5.20, 9.12–9.15, 13.5–13.6
- 2026-08-12 — **Trust chain, notifications, search, and voice coverage.** HTTPS now issues its certificate from a **local CA** the server generates once (`certs/ca.crt`, served at `/msukaip-ca.crt`); installing it on a device replaces the "Not secure" warning with a real padlock — verified by TLS handshake, which reports `UNABLE_TO_VERIFY_LEAF_SIGNATURE` without it and `authorized` with it. Leaf lifetime cut 3650 → 825 days, since browsers reject anything longer. The notification bell became a **panel** listing who is waiting with approve-in-place, Escape and click-away to close; the "Back to chat" button was removed as admins are blocked from the chat portal by design. The top-bar **search now works on every tab** (it previously reached only Users and Pending), routing to the open tab and handing Audit Logs to its server-side filter. **Voice calls gained automated coverage** (`tests/voice-call.spec.js`): two browsers, a real call, asserting ICE `connected`, DTLS `connected`, and an audio track in each direction — the last core objective that rested on a manual demo. Consent gate added to the terms modal (I Agree is inert until the box is ticked, and guarded server-side against a console bypass). Two bugs found and recorded in `DEBUGGING.md`: a popover buried by a `backdrop-filter` stacking context, and a cache that assumed an array from a route that returns 403 on purpose
